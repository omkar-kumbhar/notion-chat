...
#                 logger.info("No pages are connected to the integration.")
#             else:
#                 logger.debug(json.dumps(search_results, indent=2))  # Changed to debug level
#         elif search_results['object'] == 'error':
#             if search_results.get('status') == 401 and search_results.get('code') == 'unauthorized':
#                 logger.error("Error:", search_results['message'])
#         else:
#             logger.error("Unexpected response:", search_results)
...
#                 if block['type'] == 'paragraph' and block['paragraph']['rich_text']:
#                     content = block['paragraph']['rich_text'][0]['plain_text']
#                     logger.debug(content)  # Changed to debug level
